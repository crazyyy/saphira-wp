...
class SelfTestRedirectToConverter extends SelfTestRedirectAbstract
{
    ...
    protected function runTestForImageType($rootId, $imageType)
    {
        ...
        $htaccessFile = Paths::getAbsDirById($rootId) . '/2.htaccess';
        if (!FileHelper::fileExists($htaccessFile)) {
            $log[] = '**Warning: There is no 2.htaccess file in the ' . $rootId . ' folder!**{: .warn} (did you save settings yet?)';
            $noWarningsYet = false;
        } elseif (!HTAccess::haveWeRulesInThisHTAccess($htaccessFile)) {
            $log[] = '**Warning: There are no WebP Express rules in the 2.htaccess file in the ' . $rootId . ' folder!**{: .warn}';
            $noWarningsYet = false;
        }
...
//                $log[] = print_r($results[0]['response']['code'], true);
        ...
        if ($headers['content-type'] == 'image/webp') {
            ...
            if (PlatformInfo::isApache()) {
                $log[] = 'If not: please report this in the forum, as it seems the 2.htaccess rules ';
                ...
                $log[] = 'If not: please report this in the forum, as it seems that there is something ' .
                    'in the *2.htaccess* rules generated by WebP Express that are not working.';
            }

...
function webpexpress_migrate6_fixHtaccessRecordsForDir($dirId) {
    $haveRules = HTAccess::haveWeRulesInThisHTAccess(Paths::getAbsDirById($dirId) . '/2.htaccess');
...
function webpexpress_migrate6() {
    ...
    // Regenerate 2.htaccess file if placed in root (so rewrites does not apply in wp-admin area)
    if (HTAccess::isInActiveHTAccessDirsArray('index')) {
        if (Config::isConfigFileThere()) {
            ...
            $success = (HTAccess::saveHTAccessRulesToFile(Paths::getIndexDirAbs() . '/2.htaccess', $rules, true));

            if ($success) {
                Messenger::addMessage(
                    'info',
                    'Fixed 2.htaccess rules in root (the old rules were also applying to wp-admin folder. In some cases this resulted in problems with the media library).'
                );
            } else {
                Messenger::addMessage(
                    'warning',
                    'Tried to fix 2.htaccess rules in root folder (the old rules applied to wp-admin, which in some cases resulted in problems with media library). However, the attempt failed.'
                );
            }
        }
    }

    // The records about which 2.htaccess files that contains rules were not correct.
